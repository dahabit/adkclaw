...
├── level_1/ … level_5/ ← per-level workshop structure
│   ├── level_2/
...
│   │   ├── codelab.md  ← anchored to level_2/starter/ markers
│   │   └── README.md
│   ├── level_3/ … level_5/ (currently follow old codelab/starter + tag model)
│
├── solutions/
│   ├── level_2/        ← complete answer key (generated from v1-complete tag)
...
**The finished agent** — post-Level 5 with all 21 tools, multi-agent orchestration, healing, cron, Firestore adapter. Study this as the answer key; don't clone it as your starting point.
...
3. **Peek at `solutions/level_2/`** — the complete answer key (available after the section passes or as optional reference)

### Answer keys and diffing

`solutions/level_2/` is a **maintained answer key** — the complete, runnable finished Level 1 generated from the `v1-complete` git tag. It has:

- `solutions/level_2/src/` — all `// REPLACE-*` markers filled
- `solutions/level_2/workspace/` — a fully bootstrapped agent
- `solutions/level_2/docs/teaching-guide.md` — identical copy of this guide

Students can compare their work to the answer key:

```bash
diff -u ~/adkclaw/level_2/starter/src/agent/runner.ts ~/adkclaw/solutions/level_2/src/agent/runner.ts
```
...
|-------|-------------|---------------------|
| **Level 1** | Architecture tour | No code — presentation + repo orientation |
| **Level 2** | Brain, Tools, Personality | `agent/runner.ts`, basic tools, `IDENTITY.md`, Telegram |
| **Level 3** | Memory | `context/`, `memory/`, compaction, daily notes, bank |
| **Level 4** | Self-healing, Sub-agents | `healing/`, `multi-agent/`, cron, heartbeat, A2A |
| **Level 5** | Cloud deployment | Cloud Run, Firestore, Cloud Scheduler, webhook |
| **Level 5** | Production hardening | Admin auth, OIDC, Cloud DLP, Firestore rules, secret rotation |

Each level after Level 1 builds on the previous foundation. Self-healing is woven through Levels 4+, never a footnote.
