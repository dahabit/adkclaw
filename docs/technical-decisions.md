...
**Currency note (2026-05-08):** the legacy `gemini-2.5-pro` model is on a deprecation path with Oct 16, 2026 shutdown. Workshops running before that date can still use it; new builds should pin `gemini-3.1-pro-preview`.
...
**Currency note (2026-05-08):** legacy `gemini-2.5-flash` is on the same Oct 16, 2026 deprecation path; new builds pin `gemini-3-flash-preview`. The voice-tutor extension uses `gemini-3.1-flash-live-preview` for the bidirectional Live API.
