...
# Level 1: Architecture Tour

![Level 1: Architecture Tour](img/architecture.png)
...
- 🚀 Confidence to start building in Level 2
...
- ✅ A [Telegram bot token](https://t.me/BotFather) (used in Level 2) — send `/newbot` to BotFather
...
ls level_2/starter level_3/starter level_4/starter level_5/starter

# The post-Level-4 reference implementation (study, don't clone)
ls src/

# Per-level codelab instructions (markdown only)
ls level_1/ level_2/ level_3/ level_4/ level_5/
...
| **Self-healing** | Never crashes — recovers from any error tier | `src/healing/` | Level 4 |
| **Sub-agents** | Specialists collaborating with the main agent | `src/multi-agent/` | Level 4 |
...
> **The brain is Google. The plumbing is open source. Level 5 swaps the plumbing for Google Cloud equivalents.**

## 📁 Repo Map

| Folder | Contents | Role |
|--------|----------|------|
| `level_2/starter/` … `level_5/starter/` | Per-level self-contained starters (own `package.json`, `src/` with `//REPLACE-*` markers) | Each level: `cd level_N/starter && npm install && npm run verify` |
| `level_1/` … `level_5/` | Per-level `codelab.md` + `README.md` + `RESOURCES.md` | Codelab instructions |
| `solutions/level_2/` … `solutions/level_5/` | Answer keys (filled starters) | Diff against your starter when stuck |
...
- [ ] You've cloned the repo and `cd level_2/starter && npm install` completed
...
**[Level 1: Build the Brain →](../level_2/README.md)**
