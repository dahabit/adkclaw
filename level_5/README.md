...
- ✅ **Level 4 completed** — agent runs locally with sub-agents, healing, cron
...
**Note:** Levels 1–4 each have a self-contained per-level starter under `level_N/starter/` with offline `npm run verify` checkpoints. The answer key for this level is `solutions/level_5/`. L4 ships two REPLACE markers: `//REPLACE-FIRESTORE-LOAD` (§5) and `//REPLACE-VERIFY-OIDC` (§9). All L5 security gates are folded in — set every env var in your deploy script.
...
If you registered at [adkclaw.dev/join/sandbox](https://adkclaw.dev/join/sandbox) (see [Level 1 → Connect to the Cohort Fleet](../level_1/README.md#-optional-connect-to-the-cohort-fleet)), all four pillars light up — you're on the leaderboard. If not registered, no-op.
