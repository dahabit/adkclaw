...
id: adkclaw-codelab-4-agent-army
...
# Level 4 — The Agent Army

## Before you begin

In Level 3 your agent gained persistent memory and runtime extensibility. Today it gains **a team and a backbone**: specialised sub-agents to delegate to, a five-tier recovery pyramid that prevents crashes, cron jobs that survive restart, and a heartbeat that respects when you're sleeping. This is **Level 4 of 5** in the AdkClaw series.
...
✅ **Before starting:**
- Completed [Level 3 — Memory & Skills](https://github.com/dahabit/adkclaw/tree/main/level_3)
...
- `.env` file with `GEMINI_API_KEY` and `TELEGRAM_BOT_TOKEN` set
...
- A clone of this repo with `cd level_4/starter && npm install` done (the L3 starter ships with all L1+L2 behaviour pre-provided — no fast-forward needed)
...
**Success criteria:** `npm run verify` passes; all 132 tests green.
...
`npm run verify` should print `✓ verify passed — this checkpoint is green.` (10 test files, 132 tests). All L1+L3 behaviour ships pre-provided here — Level 4 adds four new modules (`multi-agent/`, `healing/`, `cron/`, expanded `server/`) on top.

> **Verified reference.** `solutions/level_4/` is the answer key — its `src/` is byte-identical to `level_4/starter/src/` *except* for the methods covered by `//REPLACE-*` markers. `diff -rq level_4/starter/src solutions/level_4/src` should show only the marker files differ; that's the clean-room invariant. When a snippet below is abbreviated for teaching, the solution is the source of truth.

## 1.5 Daemon startup gates (L5 hardening, folded in)

**Success criteria:** daemon boots with all three env vars set; `DAILY_TOKEN_BUDGET`, `ADMIN_KEY`, and `ALLOWED_SENDERS` validated.
...
**Success criteria:** spawned sub-agents return structured results; parent session remains unpolluted; tool allowlist enforced.
...
✅ **Section recap:** You built a `MultiAgentOrchestrator` that spawns isolated sub-agents with forked context (no parent history), tool allowlists per profile, cheaper Flash models by default (Pro only for Research/Coding), and auto-archive on completion.
...
**Success criteria:** retryable errors (rate-limit, 5xx, network) are retried with exponential backoff; non-retryable (auth, permission) escalate; Pro fallback to Flash when retries exhaust.
...
✅ **Section recap:** The recovery pyramid turns transient errors into silent retries (1s → 2s → 4s backoff) and escalates only non-recoverable errors. Pro-to-Flash fallback degrades gracefully when the primary model is overloaded.
...
**Success criteria:** cron jobs persist across restarts; idempotency prevents double-fires on restart; SQLite UNIQUE constraint enforces dedupe.
...
✅ **Section recap:** Cron jobs load from SQLite on daemon startup, fire on schedule, and dedupe missed-tick double-fires via `<jobId>:<floor(ms/60000)>` idempotency keys.
...
**Success criteria:** heartbeat respects quiet hours (22:00–07:00); no deliveries during sleep window; runner invoked when tasks are open.
...
> **Tie it to L3's Consolidator.** Level 3 introduced `src/memory/consolidator.ts` and noted "Level 4 will give us the cron engine." This is that level — but we don't auto-fire consolidation from a cron job. The natural trigger is the heartbeat: the agent reads `workspace/memory/<today>.md` on each tick and decides whether the day's notes are dense enough to promote. The agent calls `memory_save` (already wired in L3) for each durable item, or invokes `Consolidator.consolidate()` programmatically from a markdown skill. **Why not auto-schedule it?** Because consolidation is a judgment call, not an alarm — and the agent already has every tool it needs.

✅ **Section recap:** Heartbeat fires every 30 minutes during work hours and invokes the agent runner on open tasks; quiet hours block delivery between 22:00 and 07:00.
...
**Success criteria:** dashboard renders at `/`; auto-refreshes every 5 seconds; shows live session counts, token spend, cron next-fire times, and sub-agent activity.
...
✅ **Section recap:** Dashboard HTML refreshes every 5 seconds, shows active sessions, token budget, cron schedules, and sub-agent spawn activity — no build step, pure static HTML.

---

## 7. Routing & aggregation: Smarter than spawn-everything

You've built a four-profile army. But shipping **every** request to all four profiles is expensive and slow. Production systems **classify first, then route**.

**The pattern:** instead of spawning all profiles in parallel, ask yourself:

- **Search-only task?** → Dispatch to SearchAgent alone (Flash, 5 tool rounds).
- **Research + reasoning?** → ResearcherAgent (Pro, 10 rounds, memory-aware).
- **One-way notification?** → CommunicatorAgent (Flash, 3 rounds, no tools except messaging).
- **Code generation?** → CoderAgent (Pro, 8 rounds, shell + editor access).

### A lightweight router (sketch)

```typescript
// Classify the user request → which profile(s) to spawn
function classifyRequest(userText: string): string[] {
  const lower = userText.toLowerCase();
  const profiles: string[] = [];

  if (/search|google|find|latest/.test(lower)) profiles.push('search');
  if (/research|investigate|deep|analyze|why|understand/.test(lower)) {
    profiles.push('researcher');
  }
  if (/message|notify|ping|alert|send/.test(lower)) profiles.push('communicator');
  if (/code|write|implement|fix|debug|refactor/.test(lower)) profiles.push('coder');

  return profiles.length > 0 ? profiles : ['search']; // default to search
}
```

In your runner, before calling `spawnParallel()`:

```typescript
const profileIds = classifyRequest(userText);
...
**Saving:** routing cuts sub-agent spawns by ~50–80% on real workloads. Flash is cheap, but not spawning at all is cheaper.

### Aggregating results: the workspace contract

When multiple sub-agents DO run (e.g., Researcher + Communicator), define which keys each writes so the parent orchestrator knows how to merge:

| Profile | Writes to workspace | Responsibility |
|---------|-------------------|-----------------|
| SearchAgent | `facts/` | Raw search results as JSON |
| ResearcherAgent | `facts/`, `bank/` | Refined findings + saved memory |
| CommunicatorAgent | (implicit: Telegram delivery via `message_user`) | Format + send notifications |
| CoderAgent | `code/` | Generated files + diffs |

If ResearcherAgent fails mid-run but SearchAgent succeeds, the parent sees `{ ok: true, ... }` for search and `{ ok: false, error: "..." }` for researcher. You decide: proceed with partial results, retry the researcher only, or escalate.
...
const aggregated = {
  facts: results
    .filter(r => r.profileId === 'search' || r.profileId === 'researcher')
    .flatMap(r => r.summary.split('\n')), // naïve; parse JSON in production
  failures: results.filter(r => !r.ok),
  tokensUsed: results.reduce((sum, r) => sum + r.tokensUsed, 0),
};

if (aggregated.failures.length > 0) {
  // log, but don't escalate if at least one sub-agent succeeded
  console.warn(`Partial success: ${aggregated.failures.length} profiles failed`);
}
```

### A2A coordination: agent-to-agent messaging (sketch)

For sibling or grandchild delegation, teams use the **Agent-to-Agent Protocol** (open standard, Linux Foundation). It defines:

- **Message envelope:** sender ID, recipient ID, correlation ID, timestamp
- **Message types:** task request, result, heartbeat, error
- **Retry semantics:** idempotency keys for duplicate-safe re-requests

Today, your agent tree lives in one process. If you scale to multiple machines, adopt A2A for robust inter-agent messaging. The pattern is already implicit in your `spawnParallel()` calls — you're collecting results synchronously. A2A makes that async and network-resilient.

**Why not now?** Single-machine deployments (Level 4) don't need network resilience. Cloud (Level 5) introduces it.

---

✅ **Section recap:** Smart routing dispatches to only the profiles a task needs, cutting spawn overhead by 50–80%. Result aggregation defines per-profile workspace contracts so the parent merges partial results safely. Agent-to-agent coordination is a pattern for scale (Level 5+).
...
## 9. Light up your Level 4 badge

**Success criteria:** a sub-agent successfully spawns AND returns a non-error result (any of `spawn_search` / `spawn_researcher` / `spawn_communicator` / `spawn_coder`).

If you registered at [adkclaw.dev/join/sandbox](https://adkclaw.dev/join/sandbox), your third pillar lights up on the fleet view once you complete a successful spawn.
...
- A recovery pyramid that turns every transient error into a silent retry
- Cron jobs that survive restart and dedupe missed-tick double-fires
- A heartbeat that respects quiet hours (22:00–07:00)
- A dashboard for live observability
- Smart routing that dispatches only to the profiles a task needs
- A workspace aggregation pattern for safe partial-failure handling

**In practice:** ask it "Research ADK news every weekday at 9 AM, ping me only if something new shipped." It will: classify the task (researcher + communicator), spawn two profiles in parallel, merge results, dedupe via cron, and stay silent during your sleep window. That's autonomy.

---

## Glossary

- **Sub-agent:** isolated runner with different system prompt + restricted toolset
- **Forked context:** child session has no parent conversation history (identity + task only)
- **Profile:** role template (Search/Researcher/Communicator/Coder) with default model + tool allowlist
- **Recovery pyramid:** Retry → Fallback → Recover → Degrade → Escalate (5-tier resilience strategy)
- **Idempotency key:** `<jobId>:<floor(ms/60000)>` prevents double-fires on restart
- **Quiet hours:** time window (22:00–07:00) when heartbeat skips delivery
- **Result aggregation:** merging partial results from parallel sub-agents into a unified workspace state
- **Routing:** classifying a request to dispatch to only the profiles it needs

---

## Continue to Level 5

Level 5 ships your agent to **Google Cloud** so it survives losing your laptop. You'll containerise with Docker, migrate session/cron state to Firestore, switch Telegram to webhook mode, schedule cron via Cloud Scheduler, and deploy to Cloud Run. Your agent gets a public HTTPS URL.

**[Continue to Level 5 — Ship to the Cloud →](https://github.com/dahabit/adkclaw/tree/main/level_5)**
