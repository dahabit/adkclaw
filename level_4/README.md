...
![Level 4: The Agent Army](img/recovery-pyramid.png)
...
- ✅ **Level 3 completed** — agent has persistent memory + skills
- ✅ `level_4/starter/` checked out (`cd adkclaw/level_4/starter && npm install`)
...
**Note:** All levels have a self-contained per-level starter under `level_N/starter/` with offline `npm run verify` checkpoints. The answer key for this level is `solutions/level_4/`. Startup gates (DAILY_TOKEN_BUDGET, ADMIN_KEY, ALLOWED_SENDERS) are folded into L4 — set them in `.env` or the daemon won't boot.
...
If you registered at [adkclaw.dev/join/sandbox](https://adkclaw.dev/join/sandbox) (see [Level 1 → Connect to the Cohort Fleet](../level_1/README.md#-optional-connect-to-the-cohort-fleet)), your third pillar lights up on the fleet view. If not registered, no-op.
...
**[Level 5: Ship to the Cloud →](../level_5/README.md)**
